...
When QTP posts a long-running query to Slack, the alert now identifies the owning team even on queries that don't carry a `team` tag. Your tenant's existing ownership rules — equals, startsWith, contains, in, and tag matchers, composable with AND/OR — apply as a fallback whenever the query tag itself is missing.
